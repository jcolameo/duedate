...
<template>
  <div class="min-h-screen bg-slate-900 text-white p-8">
    <header class="max-w-6xl mx-auto mb-12 text-center">
      <h1 class="text-4xl font-bold text-emerald-400">📅 Deadlines</h1>
    ...
    <div class="max-w-2xl mx-auto mb-8 p-5 bg-slate-800/60 border border-slate-700 rounded-xl">
      <details class="group">
        ...
        <ol class="mt-4 space-y-2 text-sm text-slate-300 list-decimal list-inside">
          <li>Im <span class="font-mono bg-slate-900 px-1.5 py-0.5 rounded">BBNet</span> → <span
              class="font-mono bg-slate-900 px-1.5 py-0.5 rounded">Hausaufgabe</span> öffnen</li>
          <li>Oben rechts neben <span class="font-mono bg-slate-900 px-1.5 py-0.5 rounded">«Neuer Antrag»</span> auf die
            <span class="font-mono bg-slate-900 px-1.5 py-0.5 rounded">⋯</span> klicken
          </li>
          <li><span class="font-mono bg-slate-900 px-1.5 py-0.5 rounded">«Gesamte Liste exportieren»</span> wählen → CSV
            wird heruntergeladen</li>
          <li>CSV-Datei unten in die Box ziehen ⬇️</li>
          <li>Auf <span class="font-mono bg-slate-900 px-1.5 py-0.5 rounded">«In Kalender exportieren»</span> klicken
          </li>
          <li>Heruntergeladene <span class="font-mono bg-slate-900 px-1.5 py-0.5 rounded">.ics</span>-Datei öffnen →
    ...
    <div class="max-w-2xl mx-auto">
      ...
      <div v-if="errorMessage || exportError"
        class="mt-6 p-4 bg-red-900/30 border border-red-500/50 rounded-xl text-red-300">
        ⚠️ {{ errorMessage || exportError }}
      </div>

      <div v-if="exportMessage"
        class="mt-6 p-4 bg-emerald-900/30 border border-emerald-500/50 rounded-xl text-emerald-300">
        {{ exportMessage }}
      </div>
    </div>
      ...
      <div class="flex items-center justify-between mb-4 flex-wrap gap-3">
        ...
        <div class="flex items-center gap-3">
          <button v-if="isConfirmed" @click="resetMapping"
            class="px-4 py-2 text-sm bg-slate-700 hover:bg-slate-600 text-slate-200 rounded-lg transition border border-slate-600"
        ...
        <table class="w-full text-left">
          <thead class="bg-slate-800 text-slate-300 text-sm uppercase">
            <tr>
              ...
              <th class="px-4 py-3 font-semibold">{{ userMapping.category || 'Fach' }}</th>
              <th class="px-4 py-3 font-semibold">{{ userMapping.title || 'Titel' }}</th>
              <th class="px-4 py-3 font-semibold">{{ userMapping.deadline || 'Abgabe' }}</th>
              <th class="px-4 py-3 font-semibold">{{ userMapping.graded || 'Benotet' }}</th>
            </tr>
          </thead>
          <tbody class="bg-slate-900/50">
            <tr v-for="(task, index) in enrichedTasks" :key="index"
              ...
              <td class="px-4 py-3 align-top">
                <span
                  :class="['inline-block px-2 py-1 rounded-md text-xs font-bold border', STATUS_STYLES[task._status.color]]">
                  {{ task._status.label }}
                </span>
              </td>
              ...
              <td class="px-4 py-3 text-sm text-slate-300 align-top">{{ task._category || '—' }}</td>
              <td class="px-4 py-3 text-sm text-slate-200 align-top font-medium max-w-xs">{{ task._title || '—' }}
              </td>
              <td class="px-4 py-3 text-sm text-slate-400 align-top whitespace-nowrap">{{ task._deadline || '—' }}</td>
              <td class="px-4 py-3 text-sm align-top">
